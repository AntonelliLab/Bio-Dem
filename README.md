...
All CSVs in `public/data/` are generated from public sources and committed to
the repo (so the app builds and deploys without the data toolchain). To refresh
them — for example when a new V-Dem version is released:

```sh
make deps    # once: install R packages (vdemdata, sf, …) and node dependencies
make data    # regenerate all data (or: make vdem / make gbif / make colonial)
```
...
- The V-Dem data currently uses `vdemdata` v16 (years 1960–2025). Some external
  covariates end earlier than the democracy indices (education 2010, GDP 2019);
  conflict is derived from UCDP organized-violence fatalities (1989+). When a
  newer V-Dem release extends coverage, run `make data MAX_YEAR=<year>` and bump
