...
export async function downloadRecords() {
  try {
    ...
    console.log(
      `Got ${records.length} records! Writing to file '${outputFilename}'...`,
    );
    lines.unshift("country,year,records");
    await writeFile(outputFilename, lines.join("\n"));
    console.log(
      `${records.length} records written to file '${outputFilename}!`,
    );
  } catch (err) {
 ...
 * Amphibia: 131
 */
export async function downloadRecordsByTaxon(
  taxonFilter,
  { onlyDomestic = false } = {},
) {
  try {
    console.log(
      `Download records (onlyDomestic = ${onlyDomestic}) with taxon filter:`,
      taxonFilter,
    );
 ...
 */
export async function downloadRecordsByTaxons(
  taxonFilters = [],
  { addDomestic = false } = {},
) {
  try {
    if (taxonFilters.length === 0) {
      console.error(
        `No taxon filter specified. Use 'all' or empty string for all records.`,
      );
      return;
    }
    ...
    for (let filter of filters) {
      ...
      data.push(result);
      if (addDomestic) {
        const domesticResult = await downloadRecordsByTaxon(filter, {
          onlyDomestic: true,
        });
    ...
    console.log(
      `Got ${data.length} resulting datasets:`,
      JSON.stringify(
        data.map((r) => {
    ...
    data[0].records.forEach(
      (d) =>
        (recordsByCountryYear[`${d.country},${d.year}`] = {
          [data[0].name]: d.records,
        }),
    );
    // Add records from selected taxonomies
    for (let i = 1; i < data.length; ++i) {
      data[i].records.forEach(
        (d) =>
          (recordsByCountryYear[`${d.country},${d.year}`][data[i].name] =
            d.records),
      );
    }
